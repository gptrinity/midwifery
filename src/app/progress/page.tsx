...
export default async function ProgressPage() {
  ...
  const totalAnswered = attempts.reduce((a, b) => a + b.total, 0);
  ...
  // questions remaining coverage
  const coveredSubjects = new Set(attempts.filter((a) => a.subjectId).map((a) => a.subjectId));
        ...
        <div className="card p-6">
          ...
          {bySubject.size === 0 && (
            <p className="text-sm text-slate-400">
              No practice yet.{" "}
              <Link href="/practice" className="font-semibold text-brand-600 hover:underline">
                Start a session
              </Link>
            </p>
          )}
